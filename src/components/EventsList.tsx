...
const EventsList = ({ selectedCategory, selectedTimeframe }: EventsListProps) => {
  ...
  if (error) {
    return <div className="text-center py-12 text-red-500">Error loading events: {error}</div>;
  }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      {filteredEvents.map((event: any) => (
        <Card key={event.id} className="group overflow-hidden bg-white border border-gray-200/80 rounded-xl hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2">
          ...
          <CardContent className="p-6">
            ...
            <Button className="w-full font-bold bg-purple-600 hover:bg-purple-700 text-white transition-all duration-300 transform group-hover:scale-105" asChild>
